...
#pragma region CHECK_GROUP
	...
	for (uint8_t groupIndex = 0; groupIndex < pKeyGroupsCount; groupIndex++)
		for (uint8_t memberIndex = 0; memberIndex < pKeyGroupMembersCount[groupIndex]; memberIndex++)
		{
			uint8_t currentBtnIndex = toIndex(row, column);
			LOGLN("PRESSING: " + String(currentBtnIndex) + "\t" + String(groupIndex) + "," + String(memberIndex) + " = " + String(pKeyGroup[groupIndex][memberIndex]));
			//if clicked key belongs to any group
			if (currentBtnIndex == pKeyGroup[groupIndex][memberIndex])
			{
				...
				if (READ_BIT(pLEDStateArray[column], row) == 1)
					...
					for (uint8_t memberIndexAgain = 0; memberIndexAgain < pKeyGroupMembersCount[groupIndex]; memberIndexAgain++)
						if (pKeyGroup[groupIndex][memberIndexAgain] != currentBtnIndex)
						{
							uint8_t r, c;	//current row, col
							toRowCol(pKeyGroup[groupIndex][memberIndexAgain], &r, &c);
...
void MatrixKeyToLEDsClass::AddKeyGroup(uint8_t * btnArray, uint8_t size)
{
	...
	pKeyGroupMembersCount[pKeyGroupsCount] = size;
	//Copy data to array
	for (uint8_t memberIndex = 0; memberIndex < size; memberIndex++)
		pKeyGroup[pKeyGroupsCount][memberIndex] = btnArray[memberIndex];
	

#ifdef ENABLE_LOG
	LOGF("ADDED: {");
	for (uint8_t memberIndex = 0; memberIndex < pKeyGroupMembersCount[pKeyGroupsCount]; memberIndex++)
	{
		LOG(pKeyGroup[pKeyGroupsCount][memberIndex]);
		if (memberIndex < pKeyGroupMembersCount[pKeyGroupsCount] - 1)
			LOGF(",");
	}
	LOGF("} TO GROUP ");
